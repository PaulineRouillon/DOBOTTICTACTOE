    ...
    initSocket: function(){
        ...
        this.socket.onopen = function()
        {
            ...
            new QWebChannel(dobot.socket, function(channel) {
                ...
                dobot.jsApi.onExit.connect(function(poseData) {
                if (poseData)
                {
                    // 重启软件，不重载缓存区block, 退出blockly模块，再次进入重载缓存区block
                    Code.loadBlocks('');
                    dobot.jsApi.resetExitCurrentAPP();
                }
                });
                dobot.jsApi.hasExitCurrentAPP();
                $this.initJsApi();
...
//            xml.getElementsByTagName('block')[0].append(para)
            var txt = Blockly.Xml.domToText(xml);
            // txt = JSON.stringify({xml: txt, DobotType: window.currentDev})
        ...
        this.jsApi.onUpdateHWVersion.connect(function(hwVersion, currentDev) {
            window.hwVersion = hwVersion;
            window.currentDev = currentDev;
            Code.updateFromHWVersion();
        });
